	...
	if(POGEL::PHYSICS::solid_collision(obj1, obj2, &col, &vct1, &vct2, &area)) {
		...
		while(POGEL::PHYSICS::solid_collision(obj1, obj2, &col, &vct1, &vct2, &area) && bup < BUPMAX) {
			if(!obj1->hasOption(PHYSICS_SOLID_STATIONARY)) {
				...
				//obj1->rotate(POGEL::VECTOR(POGEL::MATRIX(axis,angle).getrotation()), precision/-100.0);
			}
			...
			if(!obj2->hasOption(PHYSICS_SOLID_STATIONARY)) {
				...
				//if(!POGEL::about(angle, 0.0f, precision*100))
				//obj2->rotate(POGEL::VECTOR(POGEL::MATRIX(axis,angle).getrotation()), precision/-100.0);
			}
		...
		if(!obj1->hasOption(PHYSICS_SOLID_STATIONARY)) {
			...
			//if(!obj1->sameposlegacy(precision)) {
				...
				obj1->direction /= (obj2->behavior.friction+1.0f); // compensate for friction
		...
		if(!obj2->hasOption(PHYSICS_SOLID_STATIONARY)) {
			...
			//if(!obj2->sameposlegacy(precision)) {
				...
				obj2->direction /= (obj1->behavior.friction+1.0f); // compensate for friction
